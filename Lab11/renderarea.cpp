...
void RenderArea::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if ((event->buttons() & Qt::LeftButton) && clickedShape) {
        clickedShape->move(event->pos() - clickedShapePos);
        clickedShapePos = event->pos();
    }
    update();
}
...
{
    if (event->button() == Qt::LeftButton && clickedShape) {
        clickedShape.reset();
    }
}
